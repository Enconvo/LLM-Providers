...
- Context compression is implemented in the SDK `LLMProvider`, not in provider classes here.
- Providers must preserve model context metadata from `model_registry.ts` and keep upstream context-limit error details in thrown errors.
- Providers must normalize token usage through `src/utils/usage_util.ts`; `_stream()` yields a final `usage` chunk and `_call()` attaches the same shape at `additional.metadata.providerUsage`.
- See `docs/context-compression-provider-contract.md` for the full provider contract.

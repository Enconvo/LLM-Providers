...
export class MinaiProvider extends LLMProvider {
    ...
    protected async _stream(content: LLMProvider.Params): Promise<Stream<BaseChatMessageChunk>> {
        ...
        async function* iterator(this: MinaiProvider): AsyncIterator<BaseChatMessageChunk, any, undefined> {
            if (consumed) {
            ...
            try {
                const response = await this.makeApiRequest(requestPayload, true, controller.signal);
    ...
    private buildRequestPayload(content: LLMProvider.Params, isStreaming: boolean) {
    ...
    private getFeatureType(content: LLMProvider.Params): string {
        ...
        const hasImages = content.messages?.some(msg => 
    ...
    private async makeApiRequest(payload: any, isStreaming: boolean, signal?: AbortSignal) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`1min AI API request failed with status ${response.status}: ${errorText}`);
        }

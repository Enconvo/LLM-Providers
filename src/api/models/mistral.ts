...
async function fetchModels(
  ...
  try {
    ...
    const result = await Promise.all(data.data
      .filter((item: any) => item.capabilities.completion_chat)
      .map(async (item: any) => {
        const modelId = item.id;
        const info = await getModel(modelId);

        const context = item.max_context_length || info?.maxInputTokens || 8000;
        const visionEnable = item.capabilities.vision || info?.supportsVision || false;
...
/** Mistral models list request params */
...
/**
 * Fetch and search Mistral model list

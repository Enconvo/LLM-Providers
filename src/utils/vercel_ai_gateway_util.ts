...
export const convertMessageToVercelFormat = async (
  ...
  if (message.role === "tool") {
    ...
    const contentString = typeof systemMessage.content === "string" ? systemMessage.content : systemMessage.content.map((item) => {
      if (item.type === "text") {
        return item.text
      }
  ...
  if (typeof message.content === "string") {
    ...
    const contents: (UserModelMessage | AssistantModelMessage | ToolModelMessage)[] = []

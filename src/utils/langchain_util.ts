...
export namespace LangchainUtil {
  export const convertMessageToLangchainMessage = (
      ...
      const content = message.content
        ...
        .map((item) => {
          if (item.type === "image_url") {
            ...
            const fileExists =
              url.startsWith("file://") &&
              fs.existsSync(url.replace("file://", ""));
  ...
  export const convertMessagesToLangchainMessages = (
    messages: BaseChatMessage[],
  ): BaseMessageLike[] => {
    return messages.map((message) => convertMessageToLangchainMessage(message));
  };

  export function streamFromLangchain(
    response: AsyncIterable<AIMessageChunk>,

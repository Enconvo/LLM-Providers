...
export class ChatOpenAIProvider extends LLMProvider {
  protected async _stream(
    ...
    if (this.options.originCommandName === "straico") {
      //@ts-ignore
  ...
  private async _createOpenaiClient(
    ...
    if (
      ...
      options.originCommandName === "open_ai"
    ) {
    ...
    } else if (options.originCommandName === "openrouter") {
      headers = {
        "HTTP-Referer": "https://enconvo.ai",
        "X-Title": "Enconvo",
      };
    } else if (options.originCommandName === "qwen") {
    ...
    if (options.originCommandName === "qwen" && credentialsType === "oauth2") {
      baseURL = `https://${credentials?.resource_url}/v1`;
    } else if (options.originCommandName === "straico") {
